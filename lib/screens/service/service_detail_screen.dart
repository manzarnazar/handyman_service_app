...
import 'package:booking_system_flutter/utils/dynamic_link_helper.dart';
...
// logic for description expansion
  DynamicLinkHelper dHelper = DynamicLinkHelper();
  List<Item> data = List<Item>.generate(1, (index) {
    return Item(expandedValue: "expandedValue", headerValue: "headerValue");
  });
  bool wigisExpanded = false;
      ...
      if (snap.hasError) {
        ...
        return Stack(
          children: [
            ...
            // buttons in bottom, whatsapp, phone and book now button
            Positioned(
              bottom: 16,
              left: 16,
              right: 16,
              child: Row(
                ...
                children: [
                  InkWell(
                    onTap: () async {
                      String? number = snap.data!.provider!.contactNumber;
                  ...
                  InkWell(
                    onTap: () async {
                      String? number =
                          snap.data!.provider!.contactNumber?.substring(1);
                      dHelper
                          .createDynamicLink(
                              "serviceDetails?id=${widget.serviceId.toString()}")
                          .then((dynamicLink) async {
                        final Uri whatsappUrl = Uri.parse(
                            "https://wa.me/${number}/?text=${Uri.encodeComponent(dynamicLink)}");
...
/// add this class to make the expansion logic work
class Item {
  Item({
